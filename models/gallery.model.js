...
const _artImagesByOpencall = async (id) => {
    let width, height, image_id;
    const sizeAndId = await db("opencall")
        .where({ id })
        .join("opencall_image", "opencall.id", "opencall_image.opencall_id")
        .select("opencall.max_width", "opencall.max_height", "opencall_image.image_id")
        .returning(["max_width", "max_height", "image_id"]);
    
    console.log(sizeAndId);

    width = sizeAndId[0] !== null ? sizeAndId[0].max_width : null;

    height = sizeAndId[1] !== null ? sizeAndId[0].max_height : null;

    image_id = sizeAndId[2].image_id;
